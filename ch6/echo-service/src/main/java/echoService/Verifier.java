...
import javax.security.auth.callback.UnsuppoertedCallbackException;
...
public class Verifier implements CallbackHandler {
  ...
  public void handle(Callback[] callbacks) throws UnsuportedCallbackException {
    for (int i = 0; i < callbacks.length; i++) {
      if (callback[i] instanceof PasswordValidationCallback) {
  ...
  private class PlainTextPasswordVerifier implements PasswordValidationCallback.PasswordValidator {
    public boolean validate(PasswordValidationCallback.Request req) throws PasswordValidationCallback.PasswordValidationException {
      ...
      return_username.equals(plain_pwd.getUsername()) && _password.equals(plain_pwd.getPassword());
    }
  }
}
