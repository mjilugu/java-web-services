...
public class PredictionsHttpsClientAA {
  ...
  private void getTestOne(String id, String uname, String passwd) {
    try {
  ...
  private void deleteTest(String uname, String passwd) {
    try {
      URL url = new URL(endpoint + "?id=" + id);
  ...
  private oid readResponse(String msg, HttpsURLConnection conn) {
    try {
