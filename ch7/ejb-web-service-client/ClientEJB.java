...
public class ClientEJB {
  ...
  private void runTests() {
    PredictionEJB port = new PredictionEJBService().getPredictionEJBPort();
    create(port);
    getAll(port);
    edit(port);   // id == 2
    getOne(port); // id == 2
  ...
  private getAll(PredictionEJB port) {
    System.out.println("\n### getAll:");
    List<Prediction> predicitons = port.getAll();
  ...
  private void edit(PredicitonEJB port) {
    System.out.println("\n### edit:");
  ...
  private void stringify(Prediction pred) {
    if (pred == null) return;
    String s = String.format("%2d ", pred.getId()) + pred.getWho() + ": " + pred.getWhat();
